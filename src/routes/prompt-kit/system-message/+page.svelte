...
<main class="min-w-0" bind:this={toc.ref}>
	...
	<div class="mb-6">
		<AiInstallCommand
			command="execute"
			args={["shadcn-svelte@latest", "add", `${PUBLIC_WEBSITE_URL}/p/system-message.json`]}
			component="system-message"
			registry="prompt-kit"
		/>
	</div>
	...
	<p class="text-muted-foreground mb-4 text-sm leading-relaxed sm:text-base">
		Use the <code class="bg-muted rounded px-1.5 py-0.5 text-sm">fill</code> prop to apply semantic
		background colors based on the variant. Different variants (action, warning, error) provide visual
		context for the message severity.
	</p>
	<Playground
	...
	<H3>With CTA</H3>
	<p class="text-muted-foreground mb-4 text-sm leading-relaxed sm:text-base">
		Add a call-to-action button using the <code class="bg-muted rounded px-1.5 py-0.5 text-sm"
			>cta</code
		>
	...
	<PromptKitPrevNext currentSlug="system-message" />
</main>
