<script lang="ts">
	...
	// URL for llm.txt
	const llmsTxtUrl = `${PUBLIC_WEBSITE_URL}/prompt-kit/code-block/llms.txt`;
...
<main class="min-w-0" bind:this={toc.ref}>
	<!-- Title -->
	<Subheading class="mb-4 md:text-3xl">Code Block</Subheading>
	...
	<div class="mb-6">
		<AiInstallCommand
			command="execute"
			args={["shadcn-svelte@latest", "add", `${PUBLIC_WEBSITE_URL}/p/code-block.json`]}
			component="code-block"
			registry="prompt-kit"
		/>
	</div>
	...
	<div class="bg-muted/50 my-4 rounded-lg border p-3.5">
		<p class="text-muted-foreground text-sm">
			<span class="text-foreground font-medium">💡 Tip:</span> The first three examples use the
			light theme by default. Switch to light mode to view them properly. The "Different Themes"
			example adapts to your current color mode.
		</p>
	</div>
	...
	<p class="text-muted-foreground mb-4 text-sm leading-relaxed sm:text-base">
		You can use <code class="bg-muted rounded px-1.5 py-0.5 text-sm">CodeBlockGroup</code> to add
		a header with metadata and actions to your code blocks.
	</p>
	...
	<h3 class="mt-8 mb-3 text-lg font-semibold" id="different-languages">Different Languages</h3>
	<p class="text-muted-foreground mb-4 text-sm leading-relaxed sm:text-base">
	...
	<p class="text-muted-foreground mb-4 text-sm leading-relaxed sm:text-base">
		...
		>
		and <code class="bg-muted rounded px-1.5 py-0.5 text-sm">github-dark</code> based on your current
		color mode. Try toggling your theme to see the change!
	</p>
	...
	<h3 class="mt-8 mb-3 text-xl font-semibold" id="usage-with-markdown">Usage with Markdown</h3>
	<p class="text-muted-foreground mb-4 text-sm leading-relaxed sm:text-base">
		The <code class="bg-muted rounded px-1.5 py-0.5 text-sm">CodeBlock</code> component is used
		internally by the
		...
		<code class="bg-muted rounded px-1.5 py-0.5 text-sm">not-prose</code> class to prevent conflicts
		with prose styling.
	</p>
	...
	<p class="text-muted-foreground mb-4 text-sm leading-relaxed sm:text-base">
		The <code class="bg-muted rounded px-1.5 py-0.5 text-sm">CodeBlock</code> component includes
		the <code class="bg-muted rounded px-1.5 py-0.5 text-sm">not-prose</code>
		class by default to prevent Tailwind Typography's prose styling from affecting code blocks. This
		is important when using the
		<a
			href="https://github.com/tailwindlabs/tailwindcss-typography"
			target="_blank"
			rel="noopener noreferrer"
			class="text-primary hover:underline">@tailwindcss/typography</a
		> plugin, which provides beautiful typography defaults but can interfere with code block styling.
	</p>
	<p class="text-muted-foreground mb-4 text-sm leading-relaxed sm:text-base">
		Since code blocks are styled with Shiki for syntax highlighting, they should not inherit
		Tailwind Typography styles. The <code class="bg-muted rounded px-1.5 py-0.5 text-sm"
			>not-prose</code
		> class ensures that code blocks maintain their intended appearance regardless of the surrounding
		typography context.
	</p>
	...
	<p class="text-muted-foreground mb-4 text-sm leading-relaxed sm:text-base">
		...
		>
		for syntax highlighting. You can customize the theme by passing a different theme name to the
		<code class="bg-muted rounded px-1.5 py-0.5 text-sm">theme</code> prop.
	</p>
	<p class="text-muted-foreground mb-4 text-sm leading-relaxed sm:text-base">
