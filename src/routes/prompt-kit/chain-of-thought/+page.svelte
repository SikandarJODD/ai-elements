...
<main class="min-w-0" bind:this={toc.ref}>
	...
	<div class="mb-6">
		<AiInstallCommand
			command="execute"
			args={["shadcn-svelte@latest", "add", `${PUBLIC_WEBSITE_URL}/p/chain-of-thought.json`]}
			component="chain-of-thought"
			registry="prompt-kit"
		/>
	</div>
	...
	<p class="text-muted-foreground mb-4 text-sm leading-relaxed sm:text-base">
		A more complex example showing database optimization reasoning with custom icons for each
		step and embedded code blocks. Demonstrates the full power of the component with rich
		content.
	</p>
	...
	<div class="text-muted-foreground space-y-4 text-sm leading-relaxed sm:text-base">
		...
		<div>
			...
			<p>
				Use the <code class="bg-muted rounded px-1.5 py-0.5">leftIcon</code> snippet prop on
				the trigger to add custom icons from
				<code class="bg-muted rounded px-1.5 py-0.5">@lucide/svelte</code>. By default, the
				icon swaps with a chevron on hover, but you can disable this with
		...
		<div>
			...
			<p>
				...
				code blocks, images, or other components. This makes it perfect for showing complex
				reasoning with examples.
			</p>
		</div>
	</div>

	<PromptKitPrevNext currentSlug="chain-of-thought" />
</main>
