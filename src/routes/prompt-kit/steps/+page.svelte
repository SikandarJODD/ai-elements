<script lang="ts">
	...
	// URL for llm.txt
	const llmsTxtUrl = `${PUBLIC_WEBSITE_URL}/prompt-kit/steps/llms.txt`;
...
<main class="min-w-0" bind:this={toc.ref}>
	<!-- Title -->
	<Subheading class="mb-4 md:text-3xl">Steps</Subheading>

	<!-- Description -->
	<p class="text-muted-foreground mb-6 text-base leading-relaxed sm:text-lg">
		Displays a sequence of operations in a collapsible layout. Each step can include details and
		an optional vertical bar. Useful for showing AI steps like reasoning traces, tool calls, or
		process logs.
	</p>
	...
	<Playground code={examples.basic.code} component="steps" registry="prompt-kit" source="example">
	...
	<h3 class="mt-8 mb-3 text-lg font-semibold" id="steps-with-sources">Steps with sources</h3>
	<p class="text-muted-foreground mb-4 text-sm leading-relaxed sm:text-base">
		You can use the <code class="bg-muted rounded px-1.5 py-0.5 text-sm">Source</code> component to
		display sources in the Steps block.
	</p>
	<Playground
	...
	<p class="text-muted-foreground mb-4 text-sm leading-relaxed sm:text-base">
		You can use the <code class="bg-muted rounded px-1.5 py-0.5 text-sm">Loader</code> component to
		display a text shimmer loader in the Steps block.
	</p>
	<Playground
	...
	<PromptKitPrevNext currentSlug="steps" />
</main>
