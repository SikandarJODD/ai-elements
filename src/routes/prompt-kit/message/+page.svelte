...
<Sidebar.Inset class="min-h-svh">
  ...
  >
    ...
    <main class="min-w-0" bind:this={toc.ref}>
      ...
      <h3 class="text-lg font-semibold mb-3 mt-6" id="basic-message">Basic Message</h3>
      ...
      <h3 class="text-lg font-semibold mb-3 mt-8" id="message-with-markdown">Message with Markdown</h3>
      <p class="mb-4 text-sm sm:text-base leading-relaxed text-muted-foreground">
        The <code class="bg-muted px-1.5 py-0.5 rounded text-sm">markdown</code> prop enables rendering content as Markdown, perfect for rich text formatting in messages.
      </p>
      ...
      <h3 class="text-lg font-semibold mb-3 mt-8" id="message-with-actions">Message with Actions</h3>
      <p class="mb-4 text-sm sm:text-base leading-relaxed text-muted-foreground">
        You can use <code class="bg-muted px-1.5 py-0.5 rounded text-sm">MessageActions</code> and <code
          class="bg-muted px-1.5 py-0.5 rounded text-sm">MessageAction</code
      ...
      <div class="mb-6">
        <AiInstallCommand
          command="execute"
          args={["shadcn-svelte@latest", "add", "https://ai-elements.vercel.app/r/prompt-kit-message.json"]}
        />
      </div>
      ...
      <!-- Message -->
      <ComponentAPITable componentName="Message" props={messageProps} class="mt-6" id="message-props" />

      <!-- MessageAvatar -->
      <ComponentAPITable componentName="MessageAvatar" props={messageAvatarProps} id="message-avatar-props" />

      <!-- MessageContent -->
      <ComponentAPITable componentName="MessageContent" props={messageContentProps} id="message-content-props" />

      <!-- MessageActions -->
      <ComponentAPITable componentName="MessageActions" props={messageActionsProps} id="message-actions-props" />

      <!-- MessageAction -->
      <ComponentAPITable componentName="MessageAction" props={messageActionProps} id="message-action-props" />
    </main>
