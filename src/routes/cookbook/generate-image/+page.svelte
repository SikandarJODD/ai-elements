...
<article class="mx-auto max-w-3xl px-4 py-12 md:px-6 md:py-16">
	<header class="mb-12">
		<div class="mb-6 flex items-start justify-between gap-4">
			...
			<div class="flex shrink-0 items-center gap-2">
				...
				<OpenInMenu componentName="Generate Image" {llmsTxtUrl} type="ai-elements" />
			</div>
		</div>
	...
	<section class="mb-16">
		...
		<p class="text-muted-foreground mb-6 leading-relaxed">
			The <code class="text-foreground">experimental_generateImage</code> function creates images from
			prompts. Wrap it in a tool so the AI can call it when needed.
		</p>
		<CodeNameBlock filename="tools/generate-image.ts" lang="typescript" code={toolCode} highlight={[[10, 14]]} />
	</section>

	<section class="mb-16">
		<h2 class="mb-6 text-3xl font-semibold">Server Endpoint</h2>
		<CodeNameBlock filename="+server.ts" lang="typescript" code={serverCode} highlight={[[3, 3]]} />
	</section>
	...
	<section class="mb-10">
		...
		<CodeNameBlock filename="+page.svelte" lang="svelte" code={displayCode} highlight={[[4, 7]]} />
	</section>
