...
```bash
npx shadcn-svelte@latest add https://sv-animations.vercel.app/r/queue.json
```
...
```svelte
<script lang="ts">
  import { Queue } from "$lib/components/ai-elements/queue";
</script>
...
Document the Queue props here.

## Features

- Replace this placeholder bullet with a real Queue feature.
