...
<Sidebar.Inset class="min-h-svh">
  ...
  >
    <main class="min-w-0" bind:this={toc.ref}>
      ...
      <Installation specifier={`${PUBLIC_WEBSITE_URL}/r/connection.json`} is_jsrepo={true} blockname="workflow" />
  ...
  {/if}
      ...
      <div class="space-y-4 my-4">
        <div class="border-l-2 border-primary pl-4">
          <p class="font-mono text-sm font-semibold mb-1">fromX</p>
        ...
        <div class="border-l-2 border-primary pl-4">
          <p class="font-mono text-sm font-semibold mb-1">fromY</p>
          <p class="text-sm text-muted-foreground">
        ...
        <div class="border-l-2 border-primary pl-4">
          <p class="font-mono text-sm font-semibold mb-1">toX</p>
          <p class="text-sm text-muted-foreground">
            <span class="font-mono text-xs">number</span> - The x-coordinate of the connection end
            point (follows cursor during drag).
          </p>
        </div>

        <div class="border-l-2 border-primary pl-4">
          <p class="font-mono text-sm font-semibold mb-1">toY</p>
          <p class="text-sm text-muted-foreground">
            <span class="font-mono text-xs">number</span> - The y-coordinate of the connection end
            point (follows cursor during drag).
          </p>
        </div>
      </div>
      ...
      <p class="my-4 text-sm sm:text-base text-muted-foreground">
        The Connection component creates an interactive preview while dragging from one node to
        another:
      </p>

      <ul class="space-y-3 my-4 text-sm sm:text-base">
        <li>
          <strong>Real-time Preview:</strong> Shows animated bezier curve following your cursor while
          dragging from a source node
        </li>
        <li>
          <strong>Visual Feedback:</strong> Displays an indicator circle at the cursor position showing
          where the connection will end
        </li>
        <li>
        ...
        <li>
          <strong>Theme Integration:</strong> Automatically adapts colors to your theme using CSS variables
          like <CodeSpan>--primary</CodeSpan> and <CodeSpan>--card</CodeSpan>
        </li>
        <li>
          <strong>Drop Zones:</strong> Integrates with Svelte Flow to highlight valid target nodes when
          hovering during connection creation
        </li>
      </ul>
...
@keyframes dash {
  to {
    stroke-dashoffset: -10;
      ...
      <ul class="space-y-3 my-4 text-sm sm:text-base">
        <li>
          <strong>Always use with Canvas:</strong> The Connection component requires Canvas context
          to function properly and access node positions
        </li>
        <li>
          <strong>Enable connection handles:</strong> Ensure your nodes have properly configured connection
          handles for drag interactions
        </li>
        <li>
        ...
        <li>
          <strong>Validate connections:</strong> Implement <CodeSpan>isValidConnection</CodeSpan> callback
          to prevent invalid node connections
        </li>
        <li>
          <strong>Provide user feedback:</strong> Show connection count, source/target labels, or
          status indicators after creating edges
        </li>
        <li>
          <strong>Consider accessibility:</strong> Ensure sufficient color contrast and provide keyboard
          alternatives for connection creation
        </li>
      </ul>
      ...
      <ul class="space-y-2 my-4 text-sm sm:text-base">
        <li>
          • Visual indicator circle helps users with motor impairments track cursor position during
          dragging
        </li>
        <li>
        ...
        <li>
          • Smooth animations respect user's <CodeSpan>prefers-reduced-motion</CodeSpan> system settings
        </li>
        <li>• Clear visual feedback with 2px stroke width ensures visibility on various displays</li>
        <li>
          • Works with keyboard navigation when used alongside Svelte Flow's built-in accessibility
          features
        </li>
      </ul>
    </main>
