...
```bash
npx shadcn-svelte@latest add https://sv-animations.vercel.app/r/tool.json
```
...
```svelte
<script lang="ts">
  import { Tool } from "$lib/components/ai-elements/tool";
</script>
...
Document the Tool props here.

## Features

- Replace this placeholder bullet with a real Tool feature.
