<script lang="ts">
  ...
  function insertTemplate() {
    if (headerControls) {
      headerControls.controller.textInput.setInput(
...
<div class="mx-auto max-w-4xl p-8">
  <h1 class="mb-8 text-3xl font-bold">
    PromptInput with Provider Example
  </h1>
  ...
  <PromptInputProvider initialInput="">
    ...
    <div class="mb-6 rounded-lg border bg-muted/50 p-4">
      ...
      <p class="mt-3 text-xs text-muted-foreground">
        These buttons control the prompt input from outside using
        PromptInputProvider
      </p>
    </div>
  ...
  <div class="mt-8 space-y-4 text-sm">
    <div class="rounded-lg border bg-card p-4">
      ...
      <ul class="ml-4 list-disc space-y-1 text-muted-foreground">
        <li>
          <strong>PromptInputProvider:</strong> Global state management for external
          controls
        </li>
        <li>
          <strong>Async Submit Handling:</strong> Properly handles Promise-based
          submissions
        </li>
        <li>
