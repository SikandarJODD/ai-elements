<script lang="ts">
  ...
  function getFileIcon(mediaType?: string) {
    ...
    if (mediaType.includes("pdf") || mediaType.includes("text"))
      return FileTextIcon;
    return FileIcon;
  }
...
<div class="mx-auto max-w-4xl space-y-6 p-8">
  ...
  {#if messages.length > 0}
    <div class="space-y-4 rounded-lg border bg-card p-4">
      <h3 class="font-semibold text-sm">Submission History</h3>
      <div class="space-y-3">
  ...
  <PromptInput globalDrop multiple onSubmit={handleSubmit}>
    <PromptInputBody>
      ...
      <PromptInputTextarea
        placeholder="Upload multiple files to see grouped rendering..."
      />
    </PromptInputBody>
    <PromptInputToolbar>
      <PromptInputTools>
  ...
  <div class="grid gap-4 sm:grid-cols-2">
    <div class="space-y-3 rounded-lg border bg-card p-4">
      <h4 class="font-semibold text-sm">📁 Non-Image Files</h4>
    ...
    <div class="space-y-3 rounded-lg border bg-card p-4">
      <h4 class="font-semibold text-sm">🖼️ Image Files</h4>
      <ul class="space-y-2 text-sm text-muted-foreground">
        <li class="flex gap-2">
          <span>•</span>
  ...
  <div class="rounded-lg border bg-muted/50 p-4">
    <h4 class="mb-3 font-semibold text-sm">🔧 Technical Implementation</h4>
    <div class="space-y-2 text-sm text-muted-foreground">
      <p>
        <strong class="text-foreground">Grouped Rendering:</strong> Files are automatically
        separated into two groups:
      </p>
      <ul class="ml-4 list-disc space-y-1">
        <li>
          Non-images (PDFs, docs, etc.) → rendered with icons and tooltips
        </li>
        <li>Images (PNG, JPG, etc.) → rendered as visual thumbnails</li>
      </ul>
      <p class="mt-3">
        <strong class="text-foreground">URL Conversion:</strong> Blob URLs
        (<code class="rounded bg-muted-foreground/20 px-1 text-xs"
          >blob:http://...</code
        >) are automatically converted to data URLs (<code
          class="rounded bg-muted-foreground/20 px-1 text-xs"
          >data:image/...</code
  ...
  <div class="rounded-lg bg-blue-50 p-4 dark:bg-blue-950/20">
    <p class="text-sm">
      <strong>Try it:</strong> Upload a mix of images and documents. Notice how they're
      grouped separately and displayed differently. Hover over document names to see
      the full filename and media type in a tooltip.
    </p>
  </div>
</div>
