<script lang="ts">
	...
	let controller = $state<ReturnType<typeof getPromptInputController> | null>(null);
...
<div class="mx-auto max-w-4xl space-y-6 p-8">
	...
	<PromptInputProvider initialInput="">
		<!-- Template Gallery -->
		<div class="bg-card grid gap-3 rounded-lg border p-4 sm:grid-cols-2 lg:grid-cols-4">
			{#each templates as template}
				...
				>
					...
					<p class="text-muted-foreground line-clamp-2 text-left text-xs">
						{template.text}
					</p>
				</Button>
			{/each}
		...
		<div class="bg-muted/50 rounded-lg border p-4">
			...
			<ul class="text-muted-foreground ml-4 list-disc space-y-1 text-sm">
				<li>
					Templates use <code class="text-xs">PromptInputProvider</code> to control input from
					outside
				</li>
				<li>Click any template to instantly populate the text input</li>
				<li>Character and attachment counts update in real-time</li>
				<li>The provider enables external state management without prop drilling</li>
			</ul>
		</div>
	</PromptInputProvider>
</div>
