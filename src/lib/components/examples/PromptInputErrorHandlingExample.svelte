...
<div class="mx-auto max-w-3xl space-y-6 p-8">
    ...
    <div class="flex items-start gap-3">
      ...
      <div class="flex-1">
        <div class="flex items-center justify-between">
          ...
          <button
            class="rounded-md bg-primary px-3 py-1 text-primary-foreground text-xs hover:bg-primary/90"
    ...
    <div class="rounded-lg border border-destructive bg-destructive/10 p-4">
      <div class="flex items-start gap-3">
        ...
        <div class="flex-1 space-y-2">
          <p class="font-semibold text-sm text-destructive">Submission Failed</p>
  ...
  <div class="space-y-4 rounded-lg border bg-card p-4">
    <h4 class="font-semibold text-sm">✨ Key Features Demonstrated</h4>

    <div class="space-y-3 text-sm">
      <div class="flex gap-3">
        <CheckCircle2 class="mt-0.5 h-4 w-4 shrink-0 text-green-600" />
        <div>
          <strong>Async Error Handling:</strong> If submission fails, your message
      ...
      <div class="flex gap-3">
        <CheckCircle2 class="mt-0.5 h-4 w-4 shrink-0 text-green-600" />
        <div>
          <strong>Data Persistence:</strong> Try enabling "Fail Mode" and submitting
      ...
      <div class="flex gap-3">
        <CheckCircle2 class="mt-0.5 h-4 w-4 shrink-0 text-green-600" />
        <div>
          <strong>Retry Capability:</strong> Users can fix issues and retry without
          losing work
        </div>
      </div>
      ...
      <div class="flex gap-3">
        <CheckCircle2 class="mt-0.5 h-4 w-4 shrink-0 text-green-600" />
        <div>
          <strong>Blob → Data URL:</strong> Attachments are automatically converted
          for API compatibility
        </div>
      </div>
    </div>

    <div class="mt-4 rounded-md bg-muted p-3 text-xs text-muted-foreground">
      <strong>Technical Note:</strong> The component handles both sync and async
      <code>onSubmit</code> functions. It only clears the input after successful
      completion, preventing data loss on network errors or validation failures.
    </div>
  </div>
</div>
