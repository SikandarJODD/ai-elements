...
>
  {#if mediaType === "image"}
    ...
    />
  {:else}
    <div class="flex size-full max-w-full cursor-pointer items-center justify-start gap-2 overflow-hidden px-2 text-muted-foreground">
      <PaperclipIcon class="size-4 shrink-0" />
      <Tooltip.Root delayDuration={400}>
        <Tooltip.Trigger class="min-w-0 flex-1">
          <h4 class="w-full truncate text-left font-medium text-sm">
            {data.filename || "Unknown file"}
          </h4>
        </Tooltip.Trigger>
        <Tooltip.Content>
          <div class="text-muted-foreground text-xs">
            <h4 class="max-w-[240px] overflow-hidden whitespace-normal break-words text-left font-semibold text-sm">
              {data.filename || "Unknown file"}
            </h4>
            {#if data.mediaType}
