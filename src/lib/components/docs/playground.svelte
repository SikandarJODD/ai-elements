...
<Tabs.Root bind:value={tab} class="mt-4 w-full">
	...
	<Tabs.Content value="preview" class="size-full">
		<div
			class={cn(
				"border-border relative flex min-h-[400px] w-full place-items-center justify-center rounded-lg border",
				className
			)}
		>
			{#if replay}
			...
			{/if}
			{#key remountCount}
				<div class="flex w-full place-items-center justify-center p-4 md:p-6">
					{@render children()}
				</div>
			{/key}
		</div>
