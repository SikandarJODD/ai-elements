...
const ROUTES_ROOT = path.join(ROOT_DIR, "src", "routes", "(main)", "components");
const REGISTRY_FILE = path.join(ROOT_DIR, "src", "lib", "registry", "components.ts");
...
function renderDocsMarkdown({ importStatement, title, slug, description, localName }) {
	return `# ${title}
...
function getRegistryEntryAction(existingEntries, slug) {
	return existingEntries.some((entry) => entry.id === slug) ? "update" : "create";
}
...
function buildRegistryEntries(existingEntries, routePlans) {
	...
	for (const routePlan of routePlans) {
		...
		const existingIndex = nextEntries.findIndex((entry) => entry.id === routePlan.slug);
...
export async function generateRoutes(specInputs, options = {}) {
	...
	const nextRegistryEntries = buildRegistryEntries(registryState.entries, routePlans);
	const nextRegistryContents = renderRegistryContents(registryState, nextRegistryEntries);
